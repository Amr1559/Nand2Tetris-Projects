...
public:
    ...
    void writeLabel(string label) {
        ...
        outputFile << "(" << currentFunctionName << "$" << label << ")\n";
    }
    ...
    void writeFunction(string functionName, int numVars) {
        ...
        outputFile << "// function " << functionName  << " " << numVars << "\n";
    ...
    void writeCall(string functionName, int numArgs) {
        ...
        // 1. Generate the unique return label
        string retLabel = currentFunctionName + "$ret." + to_string(callCount);
        callCount++; // Increment so the next call gets a new number!

        outputFile << "// call " << functionName << " " << numArgs << "\n";
        ...
        // 10. Declare the return address label
        outputFile << "(" << retLabel << ")\n";
    }
    ...
    void writeReturn() {
        ...
        // 1. endFrame = LCL  (R13 = endFrame)
        outputFile << "@LCL\n"
            << "D=M\n"
            << "@R13\n"
            << "M=D\n";

        // 2. retAddr = *(endFrame - 5)  (R14 = retAddr)
        outputFile << "@R13\n"
            << "D=M\n"
            << "@5\n"
            << "A=D-A\n"
